...
use crate::error::*;
use crate::{environment::*, library_name, values::*};
use crate::{
    io::file_char_stream,
    values::{ArgVec, Type},
};
    ...
    assert_eq!(
        Interpreter::eval_expression(
            &ExpressionBody::Quote(
                Box::<Datum>::new(
                    DatumBody::List(
                        vec![DatumBody::Primitive(Primitive::Integer(1)).into()]
                            .into_iter()
                            .collect()
                    )
                    .into()
